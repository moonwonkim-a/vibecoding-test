...
응답에 포함되는 `userCode7Masked` 필드는 앞 4자리 + `***` 형태로 마스킹한다.

예시: `9001151` → `9001***`

이 규칙은 모든 관리자 API 응답과 이용자 대여 확인 응답에 적용한다.
...
## 3-1. 에러코드 HTTP 상태코드 매핑

| errorCode | 상황 | HTTP Status |
|---|---|---|
| EX-001 | 대여 한도 초과 | 422 |
| EX-002 | 불량 이용자 대여 시도 | 403 |
| EX-003 | 대여 가능 재고 없음 | 409 |
| EX-004 | 연체 반납 (경고, 반납 허용) | 200 |
| EX-005 | 이용자 정보 불일치 | 404 |
| EX-006 | 검색 결과 없음 | 200 |
| EX-007 | 대여 중 재고 삭제 시도 | 409 |
| EX-008 | 관리자 로그인 실패 | 401 |
| EX-009 | 동시 대여 요청 충돌 | 409 |
| EX-010 | 잘못된 ISBN 형식 | 400 |
| EX-011 | 이미 불량 이용자 지정 시도 | 409 |
| EX-012 | 불량 사유 미선택 | 400 |
| EX-013 | 대여 이력 없는 이용자 조회 | 200 |
| EX-014 | user_code7 형식 오류 | 400 |
| EX-015 | user_code7 이름 불일치 | 409 |
| - | 미인증 관리자 API 접근 | 401 |
| - | 서버 내부 오류 | 500 |
...
인증되지 않은 요청이 `/admin/**` 엔드포인트에 접근할 경우 아래 형식으로 응답한다.
